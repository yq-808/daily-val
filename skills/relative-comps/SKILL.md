...
A single-case comps report has no scenario spread to name, so the engine builds
one from the multiples themselves: each multiple's implied price becomes a case,
and `"Bear"` / `"Bull"` resolve to the ends of that range. This is usually the
honest range for such a report anyway.
...
The fair value is always derived without reference to a market price. Where
a report carries one, it appears only after the valuation — in the implied-
expectations section and in the action band's reasoning.
